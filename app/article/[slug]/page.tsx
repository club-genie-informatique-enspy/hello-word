  ...
  useEffect(() => {
    async function initialize() {
      try {
        ...
        const fetchedArticle = await getArticle(resolvedParams.slug);
        if (!fetchedArticle) {
          console.log(`Article not found for slug: ${resolvedParams.slug}`);
        } else {
...
{/* 
          <Link className="text-xl no-underline" href={`/tags/${article.tags[0].trim().toLowerCase().replaceAll(' ', '-')}`}>
            <Button variant="link"> <Tag /> {article.tags[0]} </Button>
          </Link> */}
        </div>
      ...
      <article className="container mx-auto my-12 w-full max-w-4xl prose lg:prose-xl prose-stone dark:prose-invert">
        ...
        <div className="not-prose flex flex-wrap items-center justify-between">
          <Link href="#">
            <Button className="text-xl -px-2" variant="link"> Auteur <UserRoundPen /> </Button>
          </Link>

          {/* <div className="flex flex-row justify-between items-center">
            <time className="text-base font-light mx-1" dateTime={article.date.toString()} title={dayjs(article.date).format('DD MMM YYYY')}>
        ...
        { article && <Comments articleuuid={article.article_uuid} /> }
